...
chroot_dir=$(pwd)
cd $current_dir
init_script="/etc/rc.d/init.d/chroot-$(basename $chroot_dir)"
sed -e "/^CHROOT=/c CHROOT='$chroot_dir'" $install_dir/init-on-fedora.sh > $init_script
chmod +x $init_script
script_name=$(basename $init_script)
systemctl disable $script_name.service # it will not start automatically on reboot
