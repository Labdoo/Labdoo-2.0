...
* Installation
  ...
  + Modify and customize the settings. If you intend to build the master branch, do the following:
    #+BEGIN_EXAMPLE
    cp labdoo/install/settings.sh labdoo/cfg.sh
    vim labdoo/cfg.sh # Make any necessary changes
    #+END_EXAMPLE

  + Otherwise, if you intend to build a branch different than the master branch, do the following
    (the extra step to check out first the non-master branch is needed currently):
    #+BEGIN_EXAMPLE
    cp labdoo/install/settings.sh labdoo/cfg.sh
    cd labdoo/
