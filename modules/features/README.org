...
* Creating features
  ...
  the layout and look of a freshly installed Labdoo site):
  #+BEGIN_EXAMPLE
  variable:theme_labdoo_settings
  ...
  command like this:
  #+BEGIN_EXAMPLE
  drush @lbd features-export \
  --destination=profiles/labdoo/modules/features \
  lbd_layout $(cat layout_components)
  #+END_EXAMPLE
  It creates a feature on the directory:
  ~/var/www/lbd/profiles/labdoo/modules/features/lbd_layout/~

  Other features that I have created are: /lbd_disqus/,
  /lbd_invite/, /lbd_sharethis/,
  /lbd_captcha/, /lbd_drupalchat/,
  /lbd_janrain/, /lbd_simplenews/,
  /lbd_mass_contact/, /lbd_googleanalytics/, etc.  In
...
* Customizing features manually
  ...
  feature I added the file ~lbd_layout.install~, which uses
  *hook_enable()* to make further customizations after the feature is
  ...
  function _lbd_layout_block_settings() {
    ...
    db_query("UPDATE {block} SET title='Navigation'
	      WHERE theme='labdoo' AND delta='navig-menu'");

    // remove the title of the block user-menu
    db_query("UPDATE {block} SET title='<none>'
	      WHERE theme='labdoo' AND delta='user-menu'");
...
* Making features configurable
  ...
  installed when the (labdoo) profile is installed, and some
  ...
  administrator/maintainer of the (Labdoo) application an easy
  ...
  For example, for the feature /lbd_googleanalytics/, I have
  added this line on ~lbd_googleanalytics.module~:
  #+BEGIN_EXAMPLE
  include_once 'lbd_googleanalytics.admin.inc';
  #+END_EXAMPLE
  I have also created the file ~lbd_googleanalytics.admin.inc~
   ...
   */
  function lbd_googleanalytics_menu() {
    $items = array();
    $items['admin/config/lbd/googleanalytics'] = array(
      'title'            => 'GoogleAnalytics',
      'description'      => 'Private GoogleAnalytics settings.',
      'page callback'    => 'drupal_get_form',
      'page arguments'   => array('lbd_googleanalytics_config'),
   ...
   */
  function lbd_googleanalytics_config() {
    $form = array();
  ...
  section of Labdoo configurations. This admin/config page
