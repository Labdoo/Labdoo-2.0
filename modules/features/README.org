...
* Advantages of features
  ...
  For more details about features you can read these blogs:
...
* Creating features
  ...
  Features can be created and managed through UI. However the
  UI for creating features is often found not suitable (tedious, sluggish,
  ...
  components currently found in our Drupal site/:

  #+BEGIN_EXAMPLE
  drush @lbd help features-components
  drush @lbd features-components '%:%' > features-components.txt
  #+END_EXAMPLE
  ...
  For example, let's assume that these are the components that we select 
  for a new feature which we decide to call /lbd_sample_feature/: 
  ...
  These components include the theme, blocks layout, the front page view, and the
  menus.  Assume that we store the above lines into a file called sample_feature_components.

  Now we can create a feature that includes these components with a
  command like this:

  #+BEGIN_EXAMPLE
  drush @lbd features-export --destination=profiles/labdoo/modules/features lbd_sample_feature $(cat sample_feature_components)
  #+END_EXAMPLE

  The above command creates a feature in the directory:
  ~/var/www/lbd/profiles/labdoo/modules/features/lbd_sample_feature/~

  In order to recreate them easily, the script =create-features.sh= can
...
* Customizing features manually
  ...
  cannot get always everything right. 

  For instance, in the case of the /lbd_sample_feature/, let's assume
  that we need further customization to do certain things that the
  automatically generated feature cannot do. We can for instance add
  a ~lbd_sample_feature.install~ file and use *hook_enable()* to make 
  further customizations after the feature is installed.
   ...
   */
  function lbd_sample_feature_enable() {
    _lbd_sample_feature_create_aliases();
    _lbd_sample_feature_add_login_link();
    _lbd_sample_feature_block_settings();
  }


  function _lbd_sample_feature_create_aliases() {
    $aliases = array(
  ...
  function _lbd_sample_feature_add_login_link() {
  ...
  function _lbd_sample_feature_block_settings() {
  ...
  As an example, the above creates aliases, which cannot be handled properly by the
  ...
  handled quite well by the features), and it also sets the correct
...
* Making features configurable
  ...
  installed when the profile is installed, and some
...
* Saving and restoring private variables
  ...
  works like this:
  #+BEGIN_EXAMPLE
  features/save-private-vars.sh @lbd
  drush @lbd php-script restore-private-vars.php
  #+END_EXAMPLE
