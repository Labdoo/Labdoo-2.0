...
* Changing the webserver
  ...
  /var/www/labdoo/profiles/labdoo/dev/apache2.sh start
  /var/www/labdoo/profiles/labdoo/dev/apache2.sh stop
  #+END_EXAMPLE
...
* Re-installing the application
  ...
  #+BEGIN_EXAMPLE
  chroot /var/chroot/lbd/
  /var/www/labdoo/profiles/labdoo/dev/reinstall.sh
  #+END_EXAMPLE
  It will rebuild the Drupal directory with *drush make* and install
  the labdoo profile with *drush site-install*, and then do all
  ...
  Drupal (labdoo) and nothing else, can be done with the script
  =dev/install.sh=:
  #+BEGIN_EXAMPLE
  chroot /var/chroot/lbd/
  /var/www/labdoo/profiles/labdoo/dev/reinstall-db.sh
  #+END_EXAMPLE
...
* Making a clone for development
  ...
  #+BEGIN_EXAMPLE
  chroot /var/chroot/lbd/
  cd /var/www/labdoo/profiles/labdoo/
  dev/clone.sh dev
  #+END_EXAMPLE

  It creates a new application with root */var/www/labdoo_dev/* and
  with DB named *labdoo_dev*. It also creates the drush alias *@dev*,
  and modifies the configuration of the webserver so that the cloned
  application can be accessed at *dev.example.org*.
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  chroot /var/chroot/lbd/
  cd /var/www/
  drush php-script labdoo/profiles/labdoo/config.php dev1
  drush @dev php-script labdoo/profiles/labdoo/config.php dev2
  #+END_EXAMPLE
...
* Making a backup of the application
  ...
  chroot /var/chroot/lbd/
  cd /var/www/
  ln -s labdoo_dev/profiles/labdoo/ Labdoo

  Labdoo/dev/snapshot.sh make
  Labdoo/dev/snapshot.sh make @dev
  #+END_EXAMPLE
  These will create the files ~snapshot-labdoo-20130602.tgz~
  and ~snapshot-labdoo_dev-20130602.tgz~. They can be restored
  like this:
  #+BEGIN_EXAMPLE
  Labdoo/dev/snapshot.sh restore --file=snapshot-labdoo-20130602.tgz
  Labdoo/dev/snapshot.sh restore --file=snapshot-labdoo_dev-20130602.tgz
  Labdoo/dev/snapshot.sh restore @dev --file=snapshot-labdoo-20130602.tgz
  Labdoo/dev/snapshot.sh restore @dev --file=snapshot-labdoo_dev-20130602.tgz
  #+END_EXAMPLE
  ...
  #+BEGIN_EXAMPLE
  drush sql-dump > labdoo.sql
  drush sql-dump @dev > labdoo_dev.sql

  drush sql-query --file=$(pwd)/labdoo.sql
  drush sql-query --file=$(pwd)/labdoo_dev.sql

  drush @dev sql-query --file=$(pwd)/labdoo.sql
  drush @dev sql-query --file=$(pwd)/labdoo_dev.sql
  #+END_EXAMPLE
...
* Accessing the code of the application from outside chroot
  ...
  #+BEGIN_EXAMPLE
  cd /var/chroot/
  ln -s lbd/var/www/labdoo_dev/profiles/labdoo/ \
        dev
  #+END_EXAMPLE
...
* Pushing commits to github
  ...
  =/var/www/labdoo_dev/profiles/labdoo/= (as well as the one
  on =/var/www/labdoo/profiles/labdoo/=) are actually clones
  of the git repository at https://github.com/dashohoxha/Labdoo
...
* Making a local git clone of the dev application
  ...
  *@dev* application (~/var/www/labdoo_dev~). For example this is
  the case when I have to test install/uninstall, enable/disable
  modules, features, etc. In this case I test them on the main
  application instance (~/var/www/labdoo~). Since both
  applications are clones of the [[https://github.com/dashohoxha/Labdoo][GitHub repository]] it is easy to push
  commits from /labdoo_dev/ to /github/ and to pull them from
  /github/ to /labdoo/.
  ...
  the code on /labdoo/ is a git clone of the code on
  /labdoo_dev/ (instead of being a clone from /github/).

  The script =dev/git-clone-dev.sh= makes just this. It replaces the
  code of /labdoo/ with a git clone of the *dev branch* from
  /labdoo_dev/. Then the workflow is like this:
  1. Work and commit on the branch *dev* of *labdoo_dev*
  2. Pull on *labdoo* and test.
  3. Repeat steps *1* and *2* until the modification that we are
     making is OK.
  4. Push changes upwards to github, like this:
     #+BEGIN_EXAMPLE
     cd /var/www/labdoo_dev/profiles/labdoo
     git checkout master
     git merge dev [--squash]
     git push
     git branch -D dev
  ...
  5. Make a git pull on *labdoo* and sync it with *labdoo_dev*
     #+BEGIN_EXAMPLE
     cd /var/www/labdoo/profiles/labdoo
     git pull
     #+END_EXAMPLE
